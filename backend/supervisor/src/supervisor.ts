...
function main() {
    ...
    if (config !== undefined) {
        ...
        const api = new API(config.api.origin, config.api.port, { use: config.tls.use, cert: cert }, hostLink, databaseAdapter, config.auth.salt, config.auth.verifyLink, config.auth.passwordResetLink, transporter, config.auth.cookiehost);
        hostLink.init();
        api.init(); 
    }
}
