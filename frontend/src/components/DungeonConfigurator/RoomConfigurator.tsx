...
import { Stage, Layer, Rect, Transformer, Circle, Group, Line } from 'react-konva';
import './index.css'
...
const RoomConfigurator: React.FC<RoomConfiguratorProps> = (props) => {
    ...
    return (
        <div className="container" ref={widthRef}>
            ...
            <Stage onWheel={onWheelHandle} width={width} height={width / 1.618} draggable offsetY={-(width / 1.618)/2} offsetX={-width/2}>
                <Layer>
                    ...
                    {Object.keys(rooms).map(key => {
                        ...
                        return(
                            <Group key={key+"connections"}>
                                <Line points={[x * roomOffset + roomSize + (roomStrokeWidth / 2), y * roomOffset + (roomSize / 2), (x + 1) * roomOffset, y * roomOffset + (roomSize / 2)]} stroke={xStrokeCol} onClick={(e)=>handleConnectionClick(e, [x,y], false)} strokeWidth={connectionStrokeWidth} />
                            
                                <Line points={[x * roomOffset + (roomSize / 2), y * roomOffset + roomSize + (roomStrokeWidth / 2), x * roomOffset + (roomSize / 2), (y + 1) * roomOffset]} stroke={yStrokeCol} onClick={(e) => handleConnectionClick(e, [x, y], true)} strokeWidth={connectionStrokeWidth} />
                            </Group>
                        )
                    })}
                    </Group>
