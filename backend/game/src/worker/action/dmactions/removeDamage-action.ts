...
export class RemoveDamage implements Action {
    ...
    async performAction(user: string, args: string[]) {
        ...
        try {
            ...
            if (isNaN(dmgCount)) {
                amqpAdapter.sendActionToClient(user, "message", { message: helpMessagesForDM.valueNotANumber + dmActionDescriptions.removeDamage})
            } else {
            ...
            //console.log(e)
            amqpAdapter.sendActionToClient(user, "message", { message: parseResponseString(helpMessagesForDM.characterDoesNotExist, recipientCharacterName) + dmActionDescriptions.removeDamage })
        }
    }
}
